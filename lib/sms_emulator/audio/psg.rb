module SmsEmulator
  class PSG
    ...
    attr_reader :tone_periods, :volumes, :noise_control, :writes
    ...
    def noise_frequency
      ...
      when 0 then CLOCK / 512.0
      when 1 then CLOCK / 1024.0
      when 2 then CLOCK / 2048.0
      else tone_frequency(2)
      end
    end
    ...
    def render_samples(count, sample_rate = SAMPLE_RATE)
      ...
      count.times do |index|
        ...
        TONE_CHANNELS.times do |channel|
          ...
          mixed += (@phases[channel] < 0.5 ? 1.0 : -1.0) * volume
        end

        noise_volume = channel_volume(3)
        if noise_volume > 0.0
          advance_noise(sample_rate)
          mixed += @noise_output * noise_volume
        end

        samples[index] = (mixed / CHANNELS).clamp(-1.0, 1.0)
      end
